 ...
 */
#include "pcd8544.h"
#include "sys/circular_buffer_byte.h"

unsigned char PCD8544_Buffer[PCD8544_BUFFER_SIZE];
unsigned char PCD8544_UpdateXmin = 0, PCD8544_UpdateXmax = 0,
		PCD8544_UpdateYmin = 0, PCD8544_UpdateYmax = 0;
unsigned char PCD8544_x;
unsigned char PCD8544_y;

//uint8_t PCD8544_SPI_buffer[PCD8544_SPI_BUFFER_LENGTH];
//uint8_t PCD8544_SPI_bufferIndex = 0;
static circular_buffer_t PCD8544_SPI_buffer;
static circular_buffer_cmd_t PCD8544_SPI_TX_buffer[PCD8544_SPI_BUFFER_LENGTH];

//Fonts 5x7
const uint8_t PCD8544_Font5x7[97][PCD8544_CHAR5x7_WIDTH] = { { 0x00, 0x00, 0x00,
		0x00, 0x00 }, // sp
...
const uint8_t PCD8544_Font3x5[106][3] = { { 0x00, 0x00, 0x00 }, // sp - 32
		{ 0x00, 0x17, 0x00 }, // ! - 33
		{ 0x03, 0x00, 0x03 }, // " - 34
		{ 0x1F, 0x0A, 0x1F }, // # - 35
		{ 0x0A, 0x1F, 0x05 }, // $
...
void PCD8544_InitIO(void) {
	...
	RCC_APB2PeriphClockCmd(PCD8544_RST_RCC | PCD8544_SCE_RCC | PCD8544_DC_RCC , ENABLE);
	...
	//RST DC CE pin
	GPIO_InitStruct.GPIO_Pin = PCD8544_RST_PIN | PCD8544_DC_PIN
			| PCD8544_SCE_PIN;
	...
	GPIO_InitStruct.GPIO_Pin = PCD8544_SCK_PIN | PCD8544_MOSI_PIN;
	GPIO_InitStruct.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStruct.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_Init(PCD8544_SCK_PORT, &GPIO_InitStruct);
	...
	SPI_InitStruc.SPI_Direction = SPI_Direction_1Line_Tx; //SPI_Direction_2Lines_FullDuplex;
	SPI_InitStruc.SPI_Mode = SPI_Mode_Master;
	SPI_InitStruc.SPI_DataSize = SPI_DataSize_8b;
	SPI_InitStruc.SPI_CPOL = SPI_CPOL_High; // SPI_CPOL_High;
	SPI_InitStruc.SPI_CPHA = SPI_CPHA_2Edge; // SPI_CPHA_2Edge;
	SPI_InitStruc.SPI_NSS = SPI_NSS_Soft;
	SPI_InitStruc.SPI_FirstBit = SPI_FirstBit_MSB;
	SPI_InitStruc.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_8;
	SPI_InitStruc.SPI_CRCPolynomial = 8;
	SPI_Init(PCD8544_SPI, &SPI_InitStruc);
	...
	NVIC_InitStructure.NVIC_IRQChannel = PCD8544_SPI_IRQ;
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 1;
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	NVIC_Init(&NVIC_InitStructure);
	SPI_Cmd(PCD8544_SPI, ENABLE);
}

void PCD8544_send(PCD8544_WriteType_t cd, unsigned char data) {
	PCD8544_SCE_LOW;
//	SPI_I2S_SendData(PCD8544_SPI, data);
//	PCD8544_Delay(200);
//	PCD8544_SCE_HIGH;
	while (circular_buffer_write(&PCD8544_SPI_buffer, &cd, &data) == 0) {
		PCD8544_Delay(10);
	}

	SPI_I2S_ITConfig(PCD8544_SPI, SPI_I2S_IT_TXE, ENABLE);
}

void PCD8544_SPI_IRQHandler() {
	//
	//    If TX buffer is empty then transmit the next byte.
	//
	if (PCD8544_SPI->SR & SPI_I2S_FLAG_TXE) {
//		if (PCD8544_SPI_bufferIndex < PCD8544_SPI_BUFFER_LENGTH) {
		uint8_t data;
		PCD8544_WriteType_t cd;
		if (circular_buffer_read(&PCD8544_SPI_buffer, &cd, &data) > 0) {
			uint8_t _cd = GPIO_ReadOutputDataBit(PCD8544_DC_PORT, PCD8544_DC_PIN);
			if (cd != _cd) {
				PCD8544_DC_TOGGLE;
			}
			PCD8544_SPI->DR = data;
//			PCD8544_Delay(10); // if SPI freq is slow need to delay.
		}
	}
	//
	...
	//
	if (!(PCD8544_SPI->SR & SPI_I2S_FLAG_BSY)) {
		SPI_I2S_ITConfig(PCD8544_SPI, SPI_I2S_IT_TXE, DISABLE);
		PCD8544_SCE_HIGH;
	}
	//
	...
	//    will be regenerated.
	//
	SPI_I2S_ClearITPendingBit(PCD8544_SPI, SPI_I2S_IT_TXE);
}
...
void PCD8544_Init(unsigned char contrast) {
	//Initialize buffer
	int i = 0;
	for (i = 0; i < PCD8544_SPI_BUFFER_LENGTH-1; i++) {
		PCD8544_SPI_TX_buffer[i].buffer_cmd = PCD8544_COMMAND;
		PCD8544_SPI_TX_buffer[i].buffer_data = PCD8544_COMMAND;
	}
	circular_buffer_init(&PCD8544_SPI_buffer, &PCD8544_SPI_TX_buffer, PCD8544_SPI_BUFFER_LENGTH);
...
void PCD8544_init_LED(){
	TIM_TimeBaseInitTypeDef TIM_TimeBaseStructure;
	TIM_OCInitTypeDef TIM_OCInitStructure;
	GPIO_InitTypeDef GPIO_InitStructure;

	RCC_APB2PeriphClockCmd(PCD8544_LED_RCC, ENABLE);
	...
	GPIO_Init( PCD8544_LED_PORT , &GPIO_InitStructure);
  	...
  	// enable timer clock
	RCC_APB1PeriphClockCmd(PCD8544_LED_TIM_RCC, ENABLE);
	// configure timer
	// PWM frequency = 100 hz with 24 ,000 ,000 hz system clock
	// 24 ,000 ,000/240 = 100 ,000
	// 100 ,000/1000 = 100
	TIM_TimeBaseStructInit(&TIM_TimeBaseStructure);
	TIM_TimeBaseStructure.TIM_Prescaler = (SystemCoreClock / 50000) - 1; // 50hz
	TIM_TimeBaseStructure.TIM_Period = 1000 - 1; // 0..999
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
	TIM_TimeBaseInit(PCD8544_LED_TIM, &TIM_TimeBaseStructure);
	// PWM1 Mode configuration : Channel2
	// Edge -aligned; not single pulse mode
	TIM_OCStructInit(&TIM_OCInitStructure);
	TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1;
	TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable;
	TIM_OC2Init(PCD8544_LED_TIM, &TIM_OCInitStructure);
	// Enable Timer
	TIM_Cmd(PCD8544_LED_TIM, ENABLE);
	PCD8544_setBackground(0);
}

void PCD8544_setBackground(uint16_t value){
	TIM_SetCompare2(PCD8544_LED_TIM , value);
}
	...
	for (i = 0; i < c_width - 1; i++) {
		...
		for (j = 0; j < c_height; j++) {
			if (color == PCD8544_Pixel_Set) {
				PCD8544_DrawPixel(PCD8544_x, (PCD8544_y + j),
						((b >> j) & 1) ?
